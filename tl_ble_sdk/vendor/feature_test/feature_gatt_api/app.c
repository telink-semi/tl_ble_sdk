    ...
    case TEST_FIND_INFO_REQ:
    {
        for(int i=0; i<acl_conn_central_num; i++){
            if(conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state){
                ...
                tlkapi_printf(APP_LOG_EN,"blc_gatt_pushFindInformationRequest handle:%04X startHandle:%04X endHandle:%04X",\
                            conn_dev_list[i].conn_handle,startHandle,endHandle);
            }
        }
        app_gatt_api_test++;
        break;
    ...
    case TEST_FIND_BY_TYPE_VALUE_REQ:
    {
        for(int i=0; i<acl_conn_central_num; i++){
          if(conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state){
                    ...
                    tlkapi_printf(APP_LOG_EN,"blc_gatt_pushFindByTypeValueRequest handle:%04X startHandle:%04X endHandle:%04X UUID:0x2A50",\
                                conn_dev_list[i].conn_handle,startHandle,endHandle);
                #endif
    ...
    case TEST_READ_BY_TYPE_REQ:
    {
        for(int i=0; i<acl_conn_central_num; i++){
           if(conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state){
                ...
                tlkapi_printf(APP_LOG_EN,"blc_gatt_pushReadByTypeRequest handle:%04X startHandle:%04X endHandle:%04X otaUUID",\
                            conn_dev_list[i].conn_handle,startHandle,endHandle);
            }
        }
        app_gatt_api_test++;
    ...
    case TEST_READ_BY_GROUP_TYPE_REQ:
    {
        for(int i=0; i<acl_conn_central_num; i++){
            if(conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state){
                ...
                #if (0)
                blc_gatt_pushReadByGroupTypeRequest(conn_dev_list[i].conn_handle, startHandle, endHandle, \
                                                    (u8*)&serverDeclarationUUID, sizeof(serverDeclarationUUID));
                #elif(1)
                blc_gatt_pushReadByGroupTypeRequest(conn_dev_list[i].conn_handle, startHandle, endHandle, \
                                                    (u8*)&my_characterUUID, sizeof(my_characterUUID));

                tlkapi_printf(APP_LOG_EN,"blc_gatt_pushReadByGroupTypeRequest handle:%04X startHandle:%04X endHandle:%04X AttributeType:0x2803",\
                            conn_dev_list[i].conn_handle,startHandle,endHandle);
                #endif
            }
        }
