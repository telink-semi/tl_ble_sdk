...
u16              startHandle           = 0x0001;
u16              endHandle             = 0xFFFF;
u16              gapServerUUID         = 0x1800;
u16              serverDeclarationUUID = 0x2800;
static const u16 my_characterUUID      = DECLARATIONS_UUID_CHARACTERISTIC;
...
typedef enum
{
    TEST_WAIT_API = 0,             /*wait start*/
    TEST_FIND_INFO_REQ,
    TEST_FIND_INFO_REQ_C,          /*TEST_FIND_INFO_REQ complete*/
    TEST_FIND_BY_TYPE_VALUE_REQ,
    TEST_FIND_BY_TYPE_VALUE_REQ_C, /*TEST_FIND_BY_TYPE_VALUE_REQ complete*/
    TEST_READ_BY_TYPE_REQ,
    TEST_READ_BY_TYPE_REQ_C,       /*TEST_READ_BY_TYPE_REQ complete*/
    TEST_READ_REQ,
    TEST_READ_REQ_C,               /*TEST_READ_REQ complete*/
    TEST_READ_BLOB_REQ,
    TEST_READ_BLOB_REQ_C,          /*TEST_READ_BLOB_REQ complete*/
    TEST_READ_BY_GROUP_TYPE_REQ,
    TEST_READ_BY_GROUP_TYPE_REQ_C, /*TEST_READ_BY_GROUP_TYPE_REQ complete*/
} app_gatt_api_test_t;

app_gatt_api_test_t app_gatt_api_test = TEST_WAIT_API;

/** --------------------NOTE---------------------------------------------------
 * the feature test need hardware and firmware follow:
 * Hardware:
 *         1. EVK:    as 4 ACL Central
 *         2. dongle: as ACL Peripheral device(4)
 * Firmware:
 ...
 */
void feature_gatt_api_test_Last(void)
{
    if ((app_gatt_api_test % 2 == 0) && (app_gatt_api_test > TEST_FIND_INFO_REQ_C)) {
        app_gatt_api_test -= 3;
    }
    tlkapi_printf(APP_LOG_EN, "feature_gatt_api_test_Last :%d", app_gatt_api_test);
}

void feature_gatt_api_test_Next(void)
{
    if ((app_gatt_api_test % 2 == 0) && (app_gatt_api_test < TEST_READ_BY_GROUP_TYPE_REQ_C)) {
        app_gatt_api_test += 1;
    }
    tlkapi_printf(APP_LOG_EN, "feature_gatt_api_test_Next :%d", app_gatt_api_test);
}

void feature_gatt_api_test_mainloop(void)
{
    switch (app_gatt_api_test) {
    case TEST_WAIT_API:
    {
        break;
    }
    ...
    {
        for (int i = 0; i < acl_conn_central_num; i++) {
            if (conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state) {
                blc_gatt_pushFindInformationRequest(conn_dev_list[i].conn_handle, startHandle, endHandle);
                tlkapi_printf(APP_LOG_EN, "blc_gatt_pushFindInformationRequest handle:%04X startHandle:%04X endHandle:%04X", conn_dev_list[i].conn_handle, startHandle, endHandle);
            }
        }
        app_gatt_api_test++;
        break;
    ...
    case TEST_FIND_BY_TYPE_VALUE_REQ:
    {
        for (int i = 0; i < acl_conn_central_num; i++) {
            if (conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state) {
    #if (0)
                blc_gatt_pushFindByTypeValueRequest(conn_dev_list[i].conn_handle, startHandle, endHandle, DECLARATIONS_UUID_PRIMARY_SERVICE, (u8 *)&gapServerUUID, sizeof(gapServerUUID));
    #elif (0)
                blc_gatt_pushFindByTypeValueRequest(conn_dev_list[i].conn_handle, startHandle, endHandle, DECLARATIONS_UUID_CHARACTERISTIC, (u8 *)&my_PnCharVal, sizeof(my_PnCharVal));
    #elif (1)
                blc_gatt_pushFindByTypeValueRequest(conn_dev_list[i].conn_handle, startHandle, endHandle, CHARACTERISTIC_UUID_PNP_ID, (u8 *)&my_PnPtrs, sizeof(my_PnPtrs));
                tlkapi_printf(APP_LOG_EN, "blc_gatt_pushFindByTypeValueRequest handle:%04X startHandle:%04X endHandle:%04X UUID:0x2A50", conn_dev_list[i].conn_handle, startHandle, endHandle);
    #endif
            }
        }
        app_gatt_api_test++;
    ...
    case TEST_READ_BY_TYPE_REQ:
    {
        for (int i = 0; i < acl_conn_central_num; i++) {
            if (conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state) {
                blc_gatt_pushReadByTypeRequest(conn_dev_list[i].conn_handle, startHandle, endHandle, (u8 *)&otaUUID, sizeof(otaUUID));
                tlkapi_printf(APP_LOG_EN, "blc_gatt_pushReadByTypeRequest handle:%04X startHandle:%04X endHandle:%04X otaUUID", conn_dev_list[i].conn_handle, startHandle, endHandle);
            }
        }
        app_gatt_api_test++;
        break;
    ...
    case TEST_READ_REQ:
    {
        for (int i = 0; i < acl_conn_central_num; i++) {
            if (conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state) {
                //The customer adjusts the handle value according to the actual situation(handle:3)
                blc_gatt_pushReadRequest(conn_dev_list[i].conn_handle, 3); //device name
                tlkapi_printf(APP_LOG_EN, "blc_gatt_pushReadRequest handle:%04X Read devices name", conn_dev_list[i].conn_handle);
            }
        }
        app_gatt_api_test++;
        break;
    ...
    case TEST_READ_BLOB_REQ:
    {
        for (int i = 0; i < acl_conn_central_num; i++) {
            if (conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state) {
                ...
                tlkapi_printf(APP_LOG_EN, "blc_gatt_pushReadBlobRequest handle:%04X Read devices name", conn_dev_list[i].conn_handle);
            }
        }
        app_gatt_api_test++;
        break;
    ...
    case TEST_READ_BY_GROUP_TYPE_REQ:
    {
        for (int i = 0; i < acl_conn_central_num; i++) {
            if (conn_dev_list[i].conn_handle && conn_dev_list[i].conn_state) {
    #if (0)
                blc_gatt_pushReadByGroupTypeRequest(conn_dev_list[i].conn_handle, startHandle, endHandle, (u8 *)&serverDeclarationUUID, sizeof(serverDeclarationUUID));
    #elif (1)
                blc_gatt_pushReadByGroupTypeRequest(conn_dev_list[i].conn_handle, startHandle, endHandle, (u8 *)&my_characterUUID, sizeof(my_characterUUID));

                tlkapi_printf(APP_LOG_EN, "blc_gatt_pushReadByGroupTypeRequest handle:%04X startHandle:%04X endHandle:%04X AttributeType:0x2803", conn_dev_list[i].conn_handle, startHandle, endHandle);
    #endif
            }
        }
        app_gatt_api_test++;
...
int app_gatt_data_handler(u16 connHandle, u8 *pkt)
{
    if (dev_char_get_conn_role_by_connhandle(connHandle) == ACL_ROLE_CENTRAL) //GATT data for Central
    {
        ...
        tlkapi_printf(APP_LOG_EN, "app_gatt_data_handler opcode:%02X", pAtt->opcode);
        if (!(pAtt->opcode & 0x01)) {
            switch (pAtt->opcode) {
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] feature_gatt_api init", 0, 0);
    ////////////////////////////////////////////////////////////////////////////////////////////////
}
