...
static u32 feature_test_tick = 0;
    ...
    #define APP_LL_DLE_FLAG          0X01
    #define APP_L2CAP_MTU_FLAG       0X02
    #define APP_START_SEND_FLAG      0X04
    #define APP_FEATURE_DLE_COMPLETE (APP_LL_DLE_FLAG | APP_L2CAP_MTU_FLAG)

static u8 app_test_data[DEVICE_CHAR_INFO_MAX_NUM][251];

void feature_dle_set_param(u16 conn_idx, u8 flag)
{
    tlkapi_printf(APP_LOG_EN, "[APP][GATT]set_feature_dle_test %04X %02X", conn_idx, flag);
    feature_dle_mode[conn_idx] |= flag;
}

void feature_dle_start_test(void)
{
    tlkapi_printf(APP_LOG_EN, "[APP][GATT]feature_dle_start_test");
    feature_test_tick = clock_time() | 0x01;
}

void feature_dle_test_init(void)
{
    u8 i, j;
    //spp data init
    for (i = 0; i < (DEVICE_CHAR_INFO_MAX_NUM); i++) {
        for (j = 1; j < TEST_DATA_LEN; j++) { ///start from 1, not zero.
            app_test_data[i][j] = j - 1;
        }
        feature_dle_mode[i] = 0;
    }
    feature_test_tick = 0;
}
...
{
    u16 connHandle;
    if (feature_test_tick && clock_time_exceed(feature_test_tick, 1 * 1000 * 1000)) //1S
    {
        feature_test_tick = clock_time() | 0x01;
    #if UI_LED_ENABLE
        ...
        for (u8 i = 0; i < (DEVICE_CHAR_INFO_MAX_NUM); i++) {
            if (conn_dev_list[i].conn_state == 1) //connect state
            {
                ...
                if (feature_dle_mode[i] == APP_FEATURE_DLE_COMPLETE) {
                    if (i < ACL_CENTRAL_MAX_NUM) {
                        u16 dataLen = min(mtu_size[i] - 3, CENTRAL_ATT_RX_MTU - 3); //3 =  opcode + handle
                        ...
                        ret_val = blc_gatt_pushWriteCommand(connHandle, SPP_CLIENT_TO_SERVER_DP_H, &app_test_data[i][0], dataLen);
                        tlkapi_printf(APP_LOG_EN, "[APP][GATT] blc_gatt_pushWriteCommand     connHandle:%04X status:%02X", connHandle, ret_val);

                    } else {
                        u16 dataLen = min(mtu_size[i] - 3, PERIPHR_ATT_RX_MTU - 3);
                        u8  ret_val = BLE_SUCCESS;
                        ret_val     = blc_gatt_pushHandleValueNotify(connHandle, SPP_SERVER_TO_CLIENT_DP_H, &app_test_data[i][0], dataLen);
                        tlkapi_printf(APP_LOG_EN, "[APP][GATT]blc_gatt_pushHandleValueNotify connHandle:%04X status:%02X", connHandle, ret_val);
                    }
                    app_test_data[i][0]++;
                }
            }
...
int app_le_connection_complete_event_handle(u8 *p)
{
    ...
    if (pConnEvt->status == BLE_SUCCESS) {
        ...
        if (pConnEvt->role == ACL_ROLE_CENTRAL)         // central role, process SMP and SDP if necessary
        {
            ...
            blc_ll_sendDateLengthExtendReq(pConnEvt->connHandle, ACL_CENTRAL_MAX_TX_OCTETS);
    #endif
        }
    } else {
...
int app_disconnect_event_handle(u8 *p)
{
    ...
    u8 conn_idx = dev_char_get_conn_index_by_connhandle(pDisConn->connHandle);

    mtu_size[conn_idx] = 0;

    feature_dle_mode[conn_idx] = 0;
...
/**
 * @brief      BLE LE Data Length Change event handler
 * @param[in]  p         Pointer point to event parameter buffer.
 * @return
 */
int app_le_data_length_change_event_handle(u8 *p)
{
    hci_le_dataLengthChangeEvt_t *pLen = (hci_le_dataLengthChangeEvt_t *)p;
    tlkapi_printf(APP_CONTR_EVT_LOG_EN, "[APP][EVT] Data_Length_Change handle:%04X maxRxOct:%04X maxTxOct:%04X", pLen->connHandle, pLen->maxRxOct, pLen->maxTxOct);
    u8 conn_idx = dev_char_get_conn_index_by_connhandle(pLen->connHandle);
    feature_dle_set_param(conn_idx, APP_LL_DLE_FLAG);
    return 0;
}
    ...
    if (h & HCI_FLAG_EVENT_BT_STD) //Controller HCI event
    {
        ...
        } else if (evtCode == HCI_EVT_LE_META)         //LE Event
        {
            ...
            else if (subEvt_code == HCI_SUB_EVT_LE_CONNECTION_UPDATE_COMPLETE) // connection update
            {
                ...
                app_le_data_length_change_event_handle(p);
            }
        }
    }
...
int app_host_event_callback(u32 h, u8 *para, int n)
{
    ...
    case GAP_EVT_ATT_EXCHANGE_MTU:
    {
        ...
        tlkapi_printf(APP_CONTR_EVT_LOG_EN, "[APP][EVT] GAP_EVT_ATT_EXCHANGE_MTU handle:%04X peer_MTU:%04X effective_MTU:%04X", pEvt->connHandle, pEvt->peer_MTU, pEvt->effective_MTU);

        u8 conn_idx        = dev_char_get_conn_index_by_connhandle(pEvt->connHandle);
        mtu_size[conn_idx] = pEvt->effective_MTU; //store MTU size

        feature_dle_set_param(conn_idx, APP_L2CAP_MTU_FLAG);
    } break;
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    blc_hci_le_setEventMask_cmd(HCI_LE_EVT_MASK_CONNECTION_COMPLETE | HCI_LE_EVT_MASK_ADVERTISING_REPORT | HCI_LE_EVT_MASK_CONNECTION_UPDATE_COMPLETE | HCI_LE_EVT_MASK_DATA_LENGTH_CHANGE);
    ...
    feature_dle_test_init();
    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] TEST_LL_DLE init", 0, 0);
    ////////////////////////////////////////////////////////////////////////////////////////////////
}
