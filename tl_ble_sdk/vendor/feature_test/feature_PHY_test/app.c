...
u8 phyTest_rxfifo[UART_RX_BUFFER_SIZE * UART_RX_BUFFER_NUM] __attribute__((aligned(4))) = {0};
u8 phyTest_txfifo[UART_TX_BUFFER_SIZE * UART_TX_BUFFER_NUM] __attribute__((aligned(4))) = {0};

_attribute_ram_code_ int app_phyTest_rxUartCb(void)
{
    ...
    u8  rx_len = p[0];
    if (rx_len) {
        tlkapi_send_string_data(APP_LOG_EN, "app_phyTest_rxUartCb", &p[4], rx_len);
        ...
        blc_hci_handler(&p[4], rx_len); //the second parameter is not used.
    #endif
        bltHci_rxfifo.rptr++;
    }
    return 0;
...
static unsigned char pBuf[254];

_attribute_ram_code_ int app_phyTest_txUartCb(void)
{
    ...
    if (p) {
        u32 len = 0;
        len     = ((u16)(p)[0] + ((u16)(p)[1] << 8));
        p += 2;
        memcpy(pBuf, p, len);
        tlkapi_send_string_data(APP_LOG_EN, "app_phyTest_txUartCb", pBuf, len);
        ext_hci_uartSendData(pBuf, len);
        bltHci_txfifo.rptr++;
    }
    return 0;
}
...
///////////////////////////////////////////
_attribute_ram_code_ void app_UartTxIRQHandler(unsigned int *param)
{
    (void)*param;
    gpio_toggle(GPIO_LED_WHITE);
}

_attribute_ram_code_ void app_UartRxIRQHandler(unsigned int *param)
{
    (void)*param;
    ...
    if ((u8)(bltHci_rxfifo.wptr - bltHci_rxfifo.rptr) >= bltHci_rxfifo.num) {
        ext_hci_uartReceData((p + 4), bltHci_rxfifo.size - 4); //[!!important]

        return;                                                //have no memory.
    }
    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] app_UartRxIRQHandler ", p, 25);
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    hci_h4_uart.tx_Pin         = UART_TX_PIN; //
    hci_h4_uart.rx_Pin         = UART_RX_PIN; //
    ...
    ext_hci_uartReceData((phyTest_rxfifo + 4), UART_RX_BUFFER_SIZE - 4);
    rf_set_power_level_index(RF_POWER_P3dBm);
    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] feature_PHY_test init", 0, 0);
}
...
int main_idle_loop(void)
{
    ...
    app_phyTest_rxUartCb();
    app_phyTest_txUartCb();
