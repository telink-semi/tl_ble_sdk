...
u8 whiteList_permit_mac[6] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06};
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    //ACL Peripheral WHITE LIST Configuration - 0: ACL Peripheral role.
    //If bond number is not 0, add the latest bonding device to white list and set ADV Filter Policy to ADV_FP_ALLOW_SCAN_WL_ALLOW_CONN_WL.
    //If enable white list, only the latest bonding device can connect.
    u8 bond_number = blc_smp_param_getCurrentBondingDeviceNumber(0, 0);            //get bonded device number

    if (bond_number)                                                               //use whitelist to filter device
    {
        ...
        blc_ll_setAdvParam(ADV_INTERVAL_200MS, ADV_INTERVAL_200MS, ADV_TYPE_CONNECTABLE_UNDIRECTED, OWN_ADDRESS_PUBLIC, 0, NULL, BLT_ENABLE_ADV_ALL, ADV_FP_ALLOW_SCAN_WL_ALLOW_CONN_WL);

    } else {
        blc_ll_setAdvParam(ADV_INTERVAL_200MS, ADV_INTERVAL_200MS, ADV_TYPE_CONNECTABLE_UNDIRECTED, OWN_ADDRESS_PUBLIC, 0, NULL, BLT_ENABLE_ADV_ALL, ADV_FP_ALLOW_SCAN_ANY_ALLOW_CONN_ANY);
    }


    //ACL Central WHITE LIST Configuration - 1: ACL Central role.
    //If bond number is not 0, add the latest bonding device to white list and set SCAN Filter Policy to SCAN_FP_ALLOW_ADV_WL.
    //If enable white list, only the latest bonding device ADV can be reported and connected.
    bond_number = blc_smp_param_getCurrentBondingDeviceNumber(1, 0);               //get bonded device number
    ...
    if (bond_number)                                                               //use whitelist to filter device
    {
        ...
        blc_ll_setScanParameter(SCAN_TYPE_PASSIVE, SCAN_INTERVAL_100MS, SCAN_WINDOW_50MS, OWN_ADDRESS_PUBLIC, SCAN_FP_ALLOW_ADV_WL);
    } else {
    ...
    blc_ll_setScanEnable(BLC_SCAN_ENABLE, DUP_FILTER_DISABLE); //ADV enable
    rf_set_power_level_index(RF_POWER_P3dBm);


    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] TEST_WHITELIST init", 0, 0);
    ////////////////////////////////////////////////////////////////////////////////////////////////
}
