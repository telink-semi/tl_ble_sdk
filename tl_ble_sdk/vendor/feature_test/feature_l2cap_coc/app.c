...
int app_le_adv_report_event_handle(u8 *p)
{
    ...
    if (central_auto_connect || user_manual_pairing) {
        ...
        tlkapi_send_string_data(APP_CONTR_EVT_LOG_EN, "[APP][EVT] Create Connection", pa->mac, BLE_ADDR_LEN);

        blc_ll_createConnection(SCAN_INTERVAL_100MS, SCAN_WINDOW_100MS, INITIATE_FP_ADV_SPECIFY, pa->adr_type, pa->mac, OWN_ADDRESS_PUBLIC, CONN_INTERVAL_31P25MS, CONN_INTERVAL_48P75MS, 0, CONN_TIMEOUT_4S, 0, 0xFFFF);
    }
...
int app_le_connection_complete_event_handle(u8 *p)
{
    ...
    if (pConnEvt->status == BLE_SUCCESS) {
        tlkapi_send_string_data(APP_CONTR_EVT_LOG_EN, "[APP][EVT] Connect Complete", pConnEvt->peerAddr, BLE_ADDR_LEN);
        tlkapi_printf(APP_CONTR_EVT_LOG_EN, "[APP][EVT] Handle<0x%02X>, Interval<0x%04X>, Latency<0x%04X>, Timeout<0x%04X>\r\n", pConnEvt->connHandle, pConnEvt->connInterval, pConnEvt->peripheralLatency, pConnEvt->supervisionTimeout);
...
int app_le_connection_update_complete_event_handle(u8 *p)
{
    ...
    if (pUpt->status == BLE_SUCCESS) {
        tlkapi_printf(APP_CONTR_EVT_LOG_EN, "[APP][EVT] Connection Update Event handle<0x%04X>, Interval<0x%04X>, Latency<0x%04X>, Timeout<0x%04X>\r\n", pUpt->connHandle, pUpt->connInterval, pUpt->connLatency, pUpt->supervisionTimeout);
    }
...
int app_host_event_callback(u32 h, u8 *para, int n)
{
    ...
    case GAP_EVT_SMP_CONN_ENCRYPTION_DONE:
    {
        ...
        tlkapi_send_string_data(APP_SMP_LOG_EN, "[APP][SMP] GAP_EVT_SMP_CONN_ENCRYPTION_DONE", &pEvt->connHandle, sizeof(gap_smp_connEncDoneEvt_t));
    } break;

    case GAP_EVT_SMP_SECURITY_PROCESS_DONE:
    {
        gap_smp_connEncDoneEvt_t *pEvt = (gap_smp_connEncDoneEvt_t *)para;
        tlkapi_send_string_data(APP_SMP_LOG_EN, "[APP][SMP] GAP_EVT_SMP_SECURITY_PROCESS_DONE", &pEvt->connHandle, sizeof(gap_smp_connEncDoneEvt_t));
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    app_l2cap_coc_init();
    tlkapi_printf(APP_LOG_EN, "[APP][INI] feature_l2cap_coc started.\r\n");
    ////////////////////////////////////////////////////////////////////////////////////////////////
}
