 ...
 *  Introduction
 *  ===============
 *  TL322X supports six spi:gspi*5,lspi
 *
 *  API Reference
 *  ===============
 *  Header File: spi.h
 ...
 *  ==============
  -# SPI Initialization and Configuration
     -# nodma/dma
        - Initialize the gspi*5/lspi pin by gspi_x_set_pin() or lspi_set_pin() API;
  ...
  -# SPI_XIP
      - how to use
          ...
          - The second step initializes the pins of the spi by calling the gspi_x_set_xip_pin() or lspi_set_xip_pin() APIs.
...
extern spi_timeout_error_t g_spi_timeout_error[6];
...
#define gspi2_reset()                                                                                 spi_hw_fsm_reset(GSPI2_MODULE)
#define gspi3_reset()                                                                                 spi_hw_fsm_reset(GSPI3_MODULE)
#define gspi4_reset()                                                                                 spi_hw_fsm_reset(GSPI4_MODULE)
...
static inline void gspi_set_xip_config(spi_sel_e spi_sel, spi_xip_config_t *config)
{
    ...
    reg_gspi_rd_config(spi_sel) = (*(unsigned int *)config);
    reg_gspi_wr_config(spi_sel) = (*(((unsigned int *)config) + 1));
}
...
{
    BM_SET(reg_spi_reg_ctrl0(spi_sel), FLD_DUMMY_HOLD);
}
...
{
    BM_CLR(reg_spi_reg_ctrl0(spi_sel), FLD_DUMMY_HOLD);
}
