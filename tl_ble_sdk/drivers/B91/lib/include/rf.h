 ...
 *          Set to 1 :(1)BLE 125K and 250K mode have turned on the secondary filter, which is used to improve the chip's Out-of-Band Interference Immunity (interference including DC- offset).
...
*                        However, turning it on will tighten the chip's anti-frequency offset range to within ¡À150kHz.
 *                    (2)Secondary filter off for all modes except BLE 125K and 250K modes
 *          Set to 0: Restore the settings of the previous version's secondary filtering, only as a reserved configuration for testing, and cannot be used in actual scenarios
 *
 */
 ...
 *                                         RF global constants                                                        *
 *********************************************************************************************************************/
extern volatile rf_power_level_e rf_power_Level_list[30];   //BLE SDK use: volatile to put into RAM
 ...
 *             (2)After turning on the RX secondary filter, the anti frequency offset range of the chip will be reduced to within Â¡Ã€ 150kHz.
 */
...
/**
 *  @brief      This function serves to get rf adc iq values.
 *  @param[in]  adc_iq- Pointer to a short array to store the retrieved IQ values. Must have at least 2 elements.
 *  @return     none
*/
rf_dcoc_adc_iq_t  rf_get_adc_iq(void);  //BLE SDK use for user API ble_rf_get_adc_iq.
 ...
 *  @return     none
*/
_attribute_ram_code_sec_noinline_ void rf_set_hpmc_cal_val(unsigned short hpmc_gain);

#if (!SW_DCOC_EN)
/**
 *  @brief      This function is mainly used to get DCOC Calibration-related values.
