 ...
 *
 *******************************************************************************************************/
#ifndef __MID146085_H__
#define __MID146085_H__

/*
 * @brief     MID = 0x146085 Flash include P25Q80U.
 */


/**
 * @brief   define the section of the protected memory area which is read-only and unalterable.
 * @note    when data protection, only enumeration items in mid146085_lock_block_e can be selected,
 *          to ensure that the values returned by the interface flash_get_lock_block_mid146085 are in the mid146085_lock_block_e.
 */
typedef enum{
    //---------------- CMP bit = 0---------------//
...
}mid146085_otp_block_e;

/**
 * @brief     the lock area of the Security Registers.
 */
typedef enum{
    FLASH_LOCK_OTP_0x001000_512B_MID146085  =   0x0800, //001000h-0011FFh
    FLASH_LOCK_OTP_0x002000_512B_MID146085  =   0x1000, //002000h-0021FFh
    FLASH_LOCK_OTP_0x003000_512B_MID146085  =   0x2000, //003000h-0031FFh
 ...
 * @param[in]   mask        - mid146085_write_status_mask_e.
 ...
 * @param[in]   data    - mid146085_lock_block_e.
 * @return      1: success, 0: error, 2: parameter error.
 * @note        Attention: Before calling the FLASH function, please check the power supply voltage of the chip.
 *              Only if the detected voltage is greater than the safe voltage value, the FLASH function can be called.
 *              Taking into account the factors such as power supply fluctuations, the safe voltage value needs to be greater
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
void flash_write_otp_mid146085(unsigned long addr, unsigned long len, unsigned char *buf);
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
void flash_erase_otp_mid146085(mid146085_otp_block_e addr);
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
void flash_lock_otp_mid146085(mid146085_lock_otp_e data);
