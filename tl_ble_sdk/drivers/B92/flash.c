...
{
    DISABLE_BTB;
    flash_mspi_write_ram(FLASH_32K_BLOCK_ERASE_CMD, addr, NULL, 0);
    ENABLE_BTB;
}
...
{
    DISABLE_BTB;
    flash_mspi_write_ram(FLASH_64K_BLOCK_ERASE_CMD, addr, NULL, 0);
    ENABLE_BTB;
}
...
_attribute_text_sec_ static void flash_write(unsigned long addr, unsigned long len, unsigned char *buf, flash_command_e cmd)
{
    ...
    while (len > 0) {
        ...
        if(1 == is_flash_addr)
        {
            ...
            DISABLE_BTB;
            flash_mspi_write_ram(cmd, addr, const_buf, nw);
            ENABLE_BTB;
        }
        else
        {
