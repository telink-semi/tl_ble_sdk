 ...
 */
typedef enum {
    HAL_SD_ADC_OK = 0,         // Operation successful
    HAL_SD_ADC_ERR_PARAM,      // Parameter error (null pointer/invalid mode/missing required config)
    HAL_SD_ADC_ERR_HW_INIT,    // Hardware initialization failed (returned by chip adaptation layer)
    HAL_SD_ADC_ERR_MODE        // Mode not supported (e.g., hw_priv not configured for GPIO mode)
} hal_adc_status_e;
