...
unsigned char flash_write_status_mid1660c8(unsigned short data, unsigned int mask)
{
    ...
    unsigned short status = flash_read_status_mid1660c8();
    if(data != (status & mask))    //To reduce the operation of the status register.
    {
        ...
        status = flash_read_status_mid1660c8();
    }
    if(data == (status & mask))
    {
        return 1;
...
{
    return flash_write_status_mid1660c8(data, FLASH_WRITE_STATUS_BP_MID1660C8);
}
...
{
    return flash_write_status_mid1660c8(FLASH_LOCK_NONE_MID1660C8, FLASH_WRITE_STATUS_BP_MID1660C8);
}

/**
 * @brief       This function serves to get the protection area of the flash.
 * @return      mid1660c8_lock_block_e.
 * @note        Attention: Before calling the FLASH function, please check the power supply voltage of the chip.
 *              Only if the detected voltage is greater than the safe voltage value, the FLASH function can be called.
 *              Taking into account the factors such as power supply fluctuations, the safe voltage value needs to be greater
 *              than the minimum chip operating voltage. For the specific value, please make a reasonable setting according
...
{
    return flash_read_status_mid1660c8()&FLASH_WRITE_STATUS_BP_MID1660C8;
}
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
void flash_write_otp_mid1660c8(unsigned long addr, unsigned long len, unsigned char *buf)
{
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
void flash_erase_otp_mid1660c8(mid1660c8_otp_block_e addr)
{
...
{
    flash_write_status_mid1660c8(data, FLASH_WRITE_STATUS_OTP_MID1660C8);
}
